...
- **`zarr-global-preview` runs zones in parallel**: `--reproject-only` stops
  ...
  has no overlapping pairs through level 6 and collides at 7). Two rounds —
  odd zones, then even — cover all 60 with 30-way concurrency each, and
  `--coarsen-only` then builds levels 7-9 in one single-writer global pass.
  (@avsm)
...
- **`zarr-global-preview` writes to a remote store**: `--output` now takes a
  path *or* a URL, with its own `--output-endpoint-url`/`--output-profile`/
  `--output-region`/`--output-anon`/`--output-acl` credentials, independent
  of the source store's. The pyramid group, its level-0 chunks, the
  coarsening passes, the resume markers and the final consolidate all go
  through the same location, so embeddings can stream anonymously from a
  read-only mirror while the pyramid lands in a credentialed bucket. Zones
  composite into shared level-0 chunks with a read-modify-write, so separate
  `--zones N` invocations against one destination are safe **sequentially**
