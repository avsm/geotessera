...
class CountryLookup:
    ...
    def _get_countries_data_path(self) -> Path:
        ...
        # Download the archive through the shared downloader (retries,
        # connection pooling, atomic write).
        ...
        # Map the downloader's byte progress onto the first half of this
        # task's 0-100 scale (extraction takes the second half). GitHub may
        # serve the archive without a Content-Length, so fall back to the
        # archive's typical ~18 MB for the denominator.
