...
def download_region_file(url: str) -> str:
    ...
    try:
        ...
        temp_file.close()
        # Remove the empty placeholder so the shared downloader does a plain
        # GET (an existing file would trigger its If-Modified-Since path)
        # and writes the download into place atomically.
        os.unlink(temp_path)
