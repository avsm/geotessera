...
def init_store(
    ...
    for zone_num in sorted(landmask_by_zone.keys()):
        ...
        _create_zone_group(root, grid, output_path, model_version,
                              geotessera_version)
...
def _ensure_rgb_array(store: "zarr.Group") -> None:
    """Create the rgb array in a zone group if it doesn't exist.
    ...
    store.create_array(
        "rgb",
        shape=(T, 4, H, W),
        chunks=(1, 4, INNER_CHUNK, INNER_CHUNK),
        shards=(1, 4, SHARD_SIZE, SHARD_SIZE),
...
def _write_rgb_pass(
    ...
    def _process_shard(idx):
        ci, cj = idx
        r0 = ci * SHARD_SIZE
        r1 = min(r0 + SHARD_SIZE, H)
        c0 = cj * SHARD_SIZE
        c1 = min(c0 + SHARD_SIZE, W)
...
def _init_reproj_worker(
    ...
    global _reproj_global_arr, _reproj_src_arr
    global _reproj_to_utm, _reproj_time_index
    import zarr
    from pyproj import Transformer
    ...
    _reproj_src_arr = root[zone_group + "/rgb"]  # (T, 4, H, W)
...
def _reproject_chunk_worker(args) -> bool:
    ...
    return _reproject_chunk(
        _reproj_global_arr, chunk_row, chunk_col,
        _reproj_src_arr, _reproj_time_index,
...
def _reproject_chunk(
    ...
    """Reproject one 512x512 global chunk from an NCHW source."""
    import warnings
...
def _reproject_zone(
    ...
    """Reproject one zone's RGB into global level 0 (NCHW source)."""
    from concurrent.futures import ProcessPoolExecutor, as_completed
...
def build_global_preview(
    ...
    """Build the global EPSG:4326 RGB pyramid from zone-level RGB.
